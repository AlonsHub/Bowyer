...
// 	Using sum to handle dynamic batching!?
	float3 positionWS = positionOS + CTI_GetObjectAbsolutePositionWS();
//	Get Color Variation
	float colorVariation = saturate ( ( frac(positionWS.x + positionWS.y + positionWS.z) + frac( (positionWS.x + positionWS.y + positionWS.z) * 3.3 ) ) * 0.5 );
...
//	Wind
	...
	if (windStrength > 0) {
		...
		float4 vOscillations = AfsSmoothTriangleWave(float4(positionWS.x + sinuswave, positionWS.z + sinuswave * 0.7, positionWS.x + shiftedsinuswave, positionWS.z + shiftedsinuswave * 0.8));
...
//	From ShaderVariablesFunctions.hlsl
		...
		viewVecAbsWS = normalize(camPos - ( positionWS) );
	#else
